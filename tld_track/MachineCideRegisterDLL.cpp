// MachineCodeRegisterDLL.cpp : 定义 DLL 应用程序的导出函数。
//
...
char *disk_id()
{
	try
	{
		...
		char Name[MAX_PATH];
		CString name;
		LPWSTR LPWstring_name = new WCHAR[name.GetLength() + 1];
		DWORD serno;
		DWORD length;
		DWORD FileFlag;
		char FileName[MAX_PATH];
		CString filename=CString(FileName);
		LPWSTR LPWstring_filename = new WCHAR[filename.GetLength() + 1];
		BOOL Ret;
		Ret = GetVolumeInformation(LPCWSTR("C:\\"), LPWstring_name, MAX_PATH, &serno, &length, &FileFlag, LPWstring_filename, MAX_PATH);
